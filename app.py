...
sentry_sdk.init(
 dsn=
 "https://7c71cffadff9423a983843ddd3fe96a3@o1363527.ingest.sentry.io/4505154639364096",
 integrations=[
  FlaskIntegration(),
  PureEvalIntegration(),
 ],
...
def game():
	...
	if request.method == "GET":
		if (all(letter in session["letters"] for letter in session["word"])
...
@app.route("/game/<word>")
def game_over(word):
	return ("<h1>(っ °Д °;)っ " + (word.capitalize()) +
...
def game_success(word):
	return ("<h1> Super, du hast es erraten. Das Wort war " + word.capitalize() +
	        " </h1><script>setTimeout(function(){window.location.href = '/';}, "
	        "5000);</script><style>body{background-color: black; color: "
	        "white;}</style>")
...
def register_split():
	...
	return render_template("register_split.html",
